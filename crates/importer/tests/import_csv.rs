...
async fn it_imports_csv_and_insert_merchants_and_publish_event() {
    ...
    let job = ImportCsvJob { file_path: settings.csv_path };
    let result = import_merchants_csv_handler(job, &pool, &mock).await;
...
async fn it_imports_csv_and_upsert_existing_merchants_and_publish_event() {
    ...
    let result = import_merchants_csv_handler(job, &pool, &mock_publisher).await;
